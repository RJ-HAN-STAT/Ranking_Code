...
os.makedirs('normality/simulation_HSBM')
...
two_SBM_prob = [0.7, 0.15, 0.15]    # p_11, p_12, p_22
...
####################   Pairwise
        ...
        overall_CP_95_1 = np.int8(abs(estimation_error_1*desired_std_1) < norm.ppf(0.975)) 
        ...
        print('Average CP1 is %s'%(np.mean(overall_CP_95_1[:community[0]])))
        print('Average CP2 is %s'%(np.mean(overall_CP_95_1[community[0]:])))        
...
####################    Full Multiple 
        ...
        overall_CP_95_2 = np.int8(abs(estimation_error_2*desired_std_2) < norm.ppf(0.975)) #np.sum(abs(estimation_error_1*desired_std_1) < norm.ppf(0.975))/num_subject
        ...
        print('Average CP1 is %s'%(np.mean(overall_CP_95_2[:community[0]])))
        print('Average CP2 is %s'%(np.mean(overall_CP_95_2[community[0]:])))
...
####################    Top Multiple 
        ...
        overall_CP_95_3 = np.int8(abs(estimation_error_3*desired_std_3) < norm.ppf(0.975)) #np.sum(abs(estimation_error_1*desired_std_1) < norm.ppf(0.975))/num_subject
        ...
        print('Average CP1 is %s'%(np.mean(overall_CP_95_3[:community[0]])))
        print('Average CP2 is %s'%(np.mean(overall_CP_95_3[community[0]:])))
...
####################    Top two Multiple 
        ...
        info.to_csv('normality/simulation_HSBM/PL_inference_high_tol_%s.csv'%(n), mode = 'a', index=False, header=False)
