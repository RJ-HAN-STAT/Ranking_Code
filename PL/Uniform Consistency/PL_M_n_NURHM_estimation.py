...
os.makedirs('simulation_nonuniform')
...
L = [3, 4, 5, 6, 7]                                         # number of subjects in one comparison
tol = 1e-10
...
####################   Sample data
    ...
    for _ in range(repeat):
        num_subject = n
        num_comparison = (n**1)*int((np.log(n)**3)/10)  
        M_n = 0.5 
...
####################   Pairwise
        ...
        info.to_csv('simulation_nonuniform/PL_qmle_%s_with_%s.csv'%(n, num_comparison), mode = 'a', index=False, header=False)
...
####################    Full Multiple 
        ...
        info = pd.DataFrame(information)
        info = np.transpose(info)
        info.to_csv('simulation_nonuniform/PL_full_%s_with_%s.csv'%(n, num_comparison), mode = 'a', index=False, header=False)
...
####################    Top Multiple 
        ...
        for truncated in range(1, 3):
            ...
            info = pd.DataFrame(information)
            info = np.transpose(info)
            info.to_csv('simulation_nonuniform/PL_top_%s_%s_with_%s.csv'%(truncated,n, num_comparison), mode = 'a', index=False, header=False)
